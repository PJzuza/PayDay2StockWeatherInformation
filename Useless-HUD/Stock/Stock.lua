...
if file then
	...
	file:close()
	file = nil
	_possible_stock = assert(loadstring("return {\n".. txt .."\n}"))()
	txt = ""
end
...
function Uselesshud_Stock:GetData()
	...
	self._index_now = self._index_now or 1
	if self._index_now > #_possible_stock then
		self._index_now = 1
	end
	...
	DelayedCalls:Add("DelayedCalls_Uselesshud_Stock_Update", math.random(4, 8), function()
		if UselessHUD and self and UselessHUD.settings.Stock == 1 then
			self._index_now = self._index_now + 1
			self:GetData()
		end
	end)
end
...
function Uselesshud_Stock:SetVisible(data)
	self._uselesshud_stock_panel:set_visible(data)
	self._uselesshud_stock_text:set_visible(data)
end
